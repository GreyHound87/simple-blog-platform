...
Проект должен быть реализован на react hooks.

Для навигации по страницам - использовать react-router v5/v6?..

Адаптивная верстка не требуется.

exp и iat jwt-токена приходят в секундах

Порт у апишки 8021

Страница со списком статей / и /articles

Переход на страницу статьи при клике на заголовок.

Кнопка лайка (не активна без авторизации)

Пагинация. Должна быть на стороне сервера - при смене страницы отправляем новый запрос.
...
css свойство overflow-wrap: anywhere / break-word

При получении списка статей в запрос нужно отправлять токен, иначе статьи без проставленных лайков приходят (если пользователь авторизирован)

Страница одной статьи /articles/{slug} - Просмотр статьи с полным текстом.

Полный текст статьи - это Markdown разметка

Найти подходящий модуль для вывода содержимого на экран.

Кнопка редактирования на странице статьи. Edit.

По нажатию на Edit происходит переход на страницу редактирования.

Кнопка удаления на странице статьи. Delete.

По нажатию на Delete открытие модалки подтверждения на действие удаления

Запрос на удаление статьи

этот компонент в ant design'e называется <Popconfirm />. В документации https://ant.design/components/popconfirm в Exaples стоит самая первая ссылка "Delete". На неё нажимаете и у вас появится точь-в-точь компонент, как тот, что нужен по дизайну в фигме. Единственное, что placement={'right'} нужно указать

Отображение данных пользователя в шапке

css свойство overflow-wrap: anywhere / break-word

ограничивать длину контента, чтоб карточка не была высотой несколько тысяч пикселей
...
useEffect(() => { // проверяем корректность email и устанавливаем соответствующее значение ошибки if (email && !/\S+@\S+\.\S+/.test(email)) { setError("Данные почты некорректны"); } else { setError(""); } }, [email]);
...
Страница регистрации /sign-up - Страница регистрации.
